...
export default function Upload() {
  ...
  const thumbnailRef = useRef<HTMLDivElement | undefined>(undefined!);
  const videoRef = useRef<HTMLInputElement | undefined>(undefined!);
  ...
  const uploadThumbnail = async () => {
    ...
    const cid = await saveToIPFS(thumbnail as any);
    // Returning the CID
    return cid;
  };
  ...
  const saveVideo = async (data: any) => {
      ...
      <div className='flex-1 flex flex-col'>
        ...
        <div className='flex flex-col m-10     mt-5  lg:flex-row'>
          <div className='flex lg:w-3/4 flex-col '>
            ...
            <input
              ...
              onChange={(e) => {
                setThumbnail((e.target.files as any)[0]);
              }}
            />
          </div>
        ...
        <input
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
            setVideo((e.target.files as any)[0]);
            console.log((e.target.files as any)[0]);
          }}
        />
      </div>
    </div>
